...
class Company_BookServiceListViewModel extends ChangeNotifier {
/*
...
*/
  ...
  ApiResponse<List<Company_ServiceViewModel>> fetchServiceResponse = new ApiResponse();
  ApiResponse<List<Company_ServiceViewModel>> fetchServiceInfoResponse = new ApiResponse();

  Service service;
  void fetchServiceList() async {
    fetchServiceResponse = ApiResponse.loading();
    ApiResponse<List<Service>> _apiResponse = await WebRepository().fetchService();
    notifyListeners();
    switch(_apiResponse.status){
      case Status.EMPTY:
        fetchServiceResponse = ApiResponse.empty();
        break;
      case Status.ERROR:
        fetchServiceResponse = ApiResponse.error(message: _apiResponse.message);
        break;
      case Status.COMPLETED:
        fetchServiceResponse = ApiResponse.completed(_apiResponse.data.map((e) => Company_ServiceViewModel(service: e)).toList());
        break;
    }
    notifyListeners();
  ...
  ApiResponse<Service> serviceInfoResponse = new ApiResponse();
  void fetchServiceInformation(String id) async {
    ApiResponse<Service> _response  = await WebRepository().fetchServiceInformation(id);
    switch(_response.status){
      case Status.COMPLETED:
        serviceInfoResponse = ApiResponse.completed(_response.data);
        this.service = _response.data;
        break;
    }
    notifyListeners();
/*    current_type = null;
